...
#                               \  /  \  /     /----\    \  /    /----\
#                                \/    \/  .  /      \ .  \/  . /      \
#                                Web Application Vulnerability Analyser
#
#
#
# Tool       : WAVA v1.0
# Usage      : python3 wava.py example.com
...
def helper():
        ...
        print("------------")
        print("\t./wava.py example.com: Scans the domain example.com.")
        print("\t./wava.py example.com --skip dmitry --skip theHarvester: Skip the 'dmitry' and 'theHarvester' tests.")
        print("\t./wava.py example.com --nospinner: Disable the idle loader/spinner.")
        print("\t./wava.py --help     : Displays this help context.")
        print(bcolors.OKBLUE+"Interactive:"+bcolors.ENDC)
        print("------------")
        print("\tCtrl+C: Skips current test.")
        print("\tCtrl+Z: Quits WAVA.")
...
def logo():
    ...
    logo_ascii = """
                     ...
                     """+bcolors.OKGREEN+"""            Web Application Vulnerability Analyser """
    print(logo_ascii)
    print(bcolors.ENDC)
...
class Spinner:
    ...
    def spinner_task(self):
        ...
        try:
            ...
            print("\n\t"+ bcolors.BG_ERR_TXT+"WAVA received a series of Ctrl+C hits. Quitting..." +bcolors.ENDC)
            sys.exit(1)
...
tool_cmd   = [
                ...
                #2
                ["wget -O /tmp/wava_temp_aspnet_config_err --tries=1 ","/%7C~.aspx"],

                #3
                ["wget -O /tmp/wava_temp_wp_check --tries=1 ","/wp-admin"],

                #4
                ["wget -O /tmp/wava_temp_drp_check --tries=1 ","/user"],

                #5
                ["wget -O /tmp/wava_temp_joom_check --tries=1 ","/administrator"],
                ...
                #75
                ["wget -O /tmp/wava_temp_aspnet_elmah_axd --tries=1 ","/elmah.axd"],
                ...
                #78
                ["wapiti "," -f txt -o wava_temp_wapiti"],
...
tools_fix = [
                    ...
                    [30, "Particular Scanner found multiple vulnerabilities that an attacker may try to exploit the target.",
                            "Refer to wava-Vulnerability-Report to view the complete information of the vulnerability, once the scan gets completed."],
...
if args_namespace.help or (not args_namespace.target):
    ...
    while (rs_avail_tools < len(tools_precheck)):
        ...
        try:
            ...
            print("\t"+bcolors.BG_ERR_TXT+"WAVA was terminated abruptly..."+bcolors.ENDC)
            sys.exit(1)
    ...
    if len(unavail_tools_names) == 0:
        print("\t"+bcolors.OKGREEN+"All Scanning Tools are available. Complete vulnerability checks will be performed by WAVA."+bcolors.ENDC)
    else:
        print("\t"+bcolors.WARNING+"Some of these tools "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable or will be skipped. WAVA will still perform the rest of the tests. Install these tools to fully utilize the functionality of WAVA."+bcolors.ENDC)
    ...
    while(tool < len(tool_names)):
        ...
        temp_file = "/tmp/wava_temp_"+tool_names[tool][arg1]
        ...
        if runTest == 1:
                ...
                print("\n"+bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit WAVA.\n" + bcolors.ENDC)
                rs_skipped_checks = rs_skipped_checks + 1
    ...
    if len(rs_vul_list)==0:
        ...
        with open("wava-Vulnerability-Report", "a") as report:
            while(rs_vul < len(rs_vul_list)):
                vuln_info = rs_vul_list[rs_vul].split('*')
                report.write(vuln_info[arg2])
                report.write("\n------------------------\n\n")
                temp_report_name = "/tmp/wava_temp_"+vuln_info[arg1]
            ...
            print("\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+"`wava-Vulnerability-Report`"+bcolors.ENDC+" is available under the same directory WAVA resides.")

        report.close()
    # Writing all scan files output into wava-Debug-ScanLog for debugging purposes.
    for file_index, file_name in enumerate(tool_names):
        with open("wava-Debug-ScanLog", "a") as report:
            try:
                with open("/tmp/wava_temp_"+file_name[arg1], 'r') as temp_report:
                        data = temp_report.read()
    ...
    print("\n")
    print("\tFor Debugging Purposes, You can view the complete output generated by all the tools named "+bcolors.OKBLUE+"`wava-Debug-ScanLog`"+bcolors.ENDC+" under the same directory.")
    print(bcolors.BG_ENDL_TXT+"[ Report Generation Phase Completed. ]"+bcolors.ENDC)

    os.system('setterm -cursor on')
    os.system('rm /tmp/wava_te* > /dev/null 2>&1') # Clearing previous scan files
